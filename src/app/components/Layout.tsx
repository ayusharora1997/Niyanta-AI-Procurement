                  ...
                  <div key={item.name} className="space-y-2">
                    <button
                      ...
                      className={cn(
                        ...
                        groupActive ? 'bg-white/10 text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06)]' : 'hover:bg-white/7 hover:text-white',
                        isSidebarCollapsed && 'justify-center px-0',
                      )}
                    >
                      {groupActive ? <span className="absolute left-0 top-2.5 h-6 w-[3px] rounded-r-full bg-[#5f57ff]" /> : null}
                      <Icon className="h-[18px] w-[18px] shrink-0" />
                      {!isSidebarCollapsed ? (
                        <>
                          <span className="ml-3 truncate">{item.name}</span>
                          <ChevronDown className={cn('ml-auto h-4 w-4 text-white/55 transition-transform', isDiscoveryOpen ? 'rotate-0' : '-rotate-90')} />
                        </>
                      ) : null}
                    </button>
                          ...
                          return (
                            <Link
                              ...
                              className={cn(
                                ...
                                childActive ? 'bg-white text-[#111111]' : 'text-white/65 hover:bg-white/7 hover:text-white',
                              )}
                            >
                              {child.name}
                            </Link>
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active ? 'bg-white text-[#111111] shadow-[0_10px_28px_rgba(0,0,0,0.22)]' : 'hover:bg-white/7 hover:text-white',
                    isSidebarCollapsed && 'justify-center px-0',
                  )}
                >
                  {active ? <span className="absolute left-0 top-2.5 h-6 w-[3px] rounded-r-full bg-[#5f57ff]" /> : null}
                  <Icon className="h-[18px] w-[18px] shrink-0" />
          ...
          {!isSidebarCollapsed ? (
            <div className="mt-3 rounded-[20px] border border-white/8 bg-white/6 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-sm">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-[16px] border border-white/12 bg-[#f8f4ed]">
                  <img src="/niyanta-favicon.png" alt="Module icon" className="h-10 w-10 rounded-[12px] object-cover" />
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          active ? 'border-white bg-white text-[#111111] shadow-[0_10px_28px_rgba(0,0,0,0.18)]' : 'border-white/8 bg-black/15 text-white/72 hover:border-white/16 hover:bg-white/8 hover:text-white',
                        )}
                      >
                        <div>
                          <div className="text-[13px] font-[700]">{module.title}</div>
                          <div className={cn('text-[11px]', active ? 'text-[#6b6b6b]' : 'text-white/45')}>{module.description}</div>
                        </div>
                        {active ? <span className="h-2.5 w-2.5 rounded-full bg-[#5f57ff]" /> : null}
                      </button>
                    );
                  })}
                </div>
...
function BrandMark({ compact }: { compact: boolean }) {
  return (
    <div className={cn('flex items-center gap-3', compact && 'justify-center')}>
      <div className="relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-[16px] border border-white/10 bg-[#f8f4ed] shadow-[0_10px_24px_rgba(0,0,0,0.25)]">
        <img src="/niyanta-favicon.png" alt="Niyanta AI mark" className="h-10 w-10 rounded-[12px] object-cover" />
      </div>
      {!compact ? (
        <div>
